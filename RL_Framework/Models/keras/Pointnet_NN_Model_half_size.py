...
class NN_Model_half(tf.keras.Model):
    def __init__(self, last_layer_size):
        super().__init__()
        self.last_layer_size = last_layer_size
        self.conv_1D_11 = tf.keras.layers.Conv1D(filters=64, kernel_size=1, activation='relu', name='Conv_1_1',
                                                 input_shape=(2048, 4))
        self.conv_1D_12 = tf.keras.layers.Conv1D(filters=128, kernel_size=1, activation=None, name='Conv_1_2')

        # Pointnet Layer 2
        self.conv_1D_21 = tf.keras.layers.Conv1D(filters=256, kernel_size=1, activation='relu', name='Conv_2_1')
        self.conv_1D_22 = tf.keras.layers.Conv1D(filters=256, kernel_size=1, activation=None, name='Conv_2_2')


        # Dense Layers
        self.dense_1 = tf.keras.layers.Dense(units=64, activation='relu', name='MLP_1')
        self.dense_2 = tf.keras.layers.Dense(units=64, activation='relu', name='MLP_2')
